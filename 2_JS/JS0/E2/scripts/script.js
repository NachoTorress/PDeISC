...
function renderLista(id, array) {
  ...
  // Formato nombre_array=[dato, dato]
  contenedor.textContent = `${id}=[${array.join(', ')}]`;
}

function controlarBotones() {
  document.getElementById("btnAnimal").style.display = animales.length === 0 ? "none" : "block";
  document.getElementById("btnCompra").style.display = compras.length === 0 ? "none" : "block";
  document.getElementById("btnNumero").style.display = numeros.length === 0 ? "none" : "block";
}
...
function vaciarNumeros() {
  ...
  document.getElementById("resultado").textContent = `Se eliminaron: ${eliminados.join(", ")}`;
  actualizarPantalla();
}
...
document.getElementById("btnAnimal").addEventListener("click", () => dobleToque(eliminarAnimal));
document.getElementById("btnCompra").addEventListener("click", () => dobleToque(eliminarProducto));
document.getElementById("btnNumero").addEventListener("click", () => dobleToque(vaciarNumeros));
