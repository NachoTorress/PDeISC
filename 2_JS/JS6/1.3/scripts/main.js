...
import { pedirPalabra, guardarPuntaje, descargarPdf, descargarTablaPdf, obtenerTablaPosiciones } from './api.js';
...
import { renderizarTablaPosiciones, renderizarTop3 } from './leaderboard.js';
...
const elBotonDescargarTabla = document.getElementById('btn-descargar-tabla-pdf');
...
// ---- Evento: descargar tabla completa (historica) como PDF ----
elBotonDescargarTabla.addEventListener('click', async () => {
    elBotonDescargarTabla.disabled = true;
    try {
        await descargarTablaPdf();
    } catch (error) {
        console.error(error);
    } finally {
        elBotonDescargarTabla.disabled = false;
    }
});
