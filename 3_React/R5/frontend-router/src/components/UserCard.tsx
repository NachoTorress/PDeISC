...
export function UserCard({ user, currentUser, onUpdate, onDelete }: UserCardProps) {
  ...
  return (
    <article className="user-card">
      <div className="user-card-header d-flex align-items-center gap-3">
        {user.avatarUrl ? (
          <img
            src={user.avatarUrl}
        ...
        <div className="badge-stack">
          {user.provider && user.provider !== 'local' ? (
            <span className="badge text-bg-info text-capitalize me-1">{user.provider}</span>
          ) : null}
      ...
      {editing ? (
        ...
        <>
          <dl className="user-data-grid mt-3">
            <div>
              <dt>Documento</dt>
              <dd>
            ...
            <div>
              <dt>Actualizado</dt>
              <dd>{formatDateDDMMYY(user.updatedAt)}</dd>
            </div>
          </dl>
          ...
          {confirmingDelete ? (
            <div className="delete-confirmation">
              <p>Esta acción elimina la cuenta y su perfil. No se puede deshacer.</p>
